<script lang="ts">
	...
	const childrenNames: any[] = rowsArray.flatMap((row) =>
			...
			{#each depthArray as depth}
				<Mesh
					...
					onMouseleave={({ target, scene }) => {
						...
						if (group) {
							const neighbours = GridHelper.getNeighboursOfTarget(group, target, cellDistance, cellSize)
